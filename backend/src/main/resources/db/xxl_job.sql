...
CREATE TABLE IF NOT EXISTS `xxl_job_log_report_cache` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `trigger_day` datetime DEFAULT NULL COMMENT '调度日期',
  `running_count` int(11) NOT NULL DEFAULT '0' COMMENT '运行中-日志数量',
  `suc_count` int(11) NOT NULL DEFAULT '0' COMMENT '执行成功-日志数量',
...
VALUES (1, 'market-order-executor', '订单执行器', 0, NULL, NOW())
...
VALUES (
  1, 1, '整点扫描已支付超过1小时的订单并自动退款', NOW(), NOW(), 'system', '',
  'CRON', '0 0 * * * ?', 'DO_NOTHING', 'FIRST',
  'expirePaidVoucherOrdersJob', '', 'SERIAL_EXECUTION', 0,
  0, 'BEAN', '', 'init', NOW(),
  '', 1, 0, 0
)
ON DUPLICATE KEY UPDATE
